...
func (m *MemoryMonitor) Check() []Alert {
	...
	if usage >= m.threshold {
		...
		return []Alert{
			{
				Type:     "memory",
				Message:  fmt.Sprintf("Memory usage is %.1f%% (%.1f MB / %.1f MB), exceeding threshold of %.1f%%%s", usage, usedMB, totalMB, m.threshold, details),
				Value:    usage,
				Severity: severity,
			},
		}
